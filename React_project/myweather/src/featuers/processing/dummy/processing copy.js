...
export const getHome = ({ weather, forecast, airPollution }) => {
   const timezoneOffset = weather.timezone // 타임존 보정값
   const localWeatherTime = convertToLocalTime(weather.dt, timezoneOffset) // 현재 로컬 시간
   const remainingHours = calculateRemainingHours(localWeatherTime) // 오늘 남은 시간

   // Hourly 데이터 생성
   const hourly = generateHourlyData(weather, forecast, timezoneOffset)

   // 데일리 데이터 생성
   const daily = groupDailyData(forecast.list.slice(Math.ceil(remainingHours / 3)), timezoneOffset)

   // 최종 데이터 반환
   const now = {
      id: weather.id,
      name: weather.name,
      date: new Date(localWeatherTime * 1000).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' }),
   ...
   return {
      ...
      air,
      hourly,
      daily: daily.slice(0, 4), // 최대 4일치 반환
   }
}
