const styleDisplayKeys = (as, display) => {
    ...
    const validDisplay = typeToDisplayMap[as] || display

    const allValidDisplays = Object.values(displayGroups).flat()
    if (!allValidDisplays.includes(validDisplay)) {
        throw new Error(`Invalid display value "${validDisplay}" for type "${as}". Please provide a valid display.`)
    }
